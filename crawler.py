...
class SearchEngineCrawler:
    def __init__(self):
        ...
        if not self.google_search_engine_id:
            self.logger.warning("GOOGLE_SEARCH_ENGINE_ID environment variable not set")

    def search_google_custom(self, query, num_results=10):
        """Search using Google Custom Search JSON API"""
        if not self.google_api_key or not self.google_search_engine_id:
            self.logger.error("Google API key or Search Engine ID not provided in environment variables")
            return []
    ...
    def unified_search(self, query, num_results=10, engines=None):
        ...
        if engines is None:
            engines = ['bing', 'duckduckgo']  # Default to free engines
...
if __name__ == "__main__":
    # Initialize the crawler - it automatically reads from .env file
